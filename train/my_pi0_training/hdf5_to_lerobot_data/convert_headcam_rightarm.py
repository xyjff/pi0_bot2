...
# 4、一键启动转换脚本       python /share/0xyj/model3_openpi0.5/my_pi0_training/hdf5_to_lerobot_data/convert_headcam_rightarm.py
...
def process_dataset():
    ...
    print(f"\n✅ 转换完成！只保留了头部摄像头 + 右臂右灵巧手（8自由度）")
    print(
        f"📊 总耗时: {format_seconds(total_elapsed)} | "
        f"总剧集: {total_episodes} | "
        f"总帧数: {total_frames}"
    )
    print(f"\n📁 输出目录: {OUTPUT_DIR}")
